...
public class KidsWithGreatestNumberOfCandies {
    ...
    public List<Boolean> kidsWithCandies(int[] candies, int extraCandies) {
        ...
        Boolean[] hasGreatestNumberOfCandiesArray = new Boolean[candies.length];
        int currentMaxCandy = findCurrentMaxCandy(candies);

        for(int i = 0; i < candies.length; i++) {

            if (candies[i] + extraCandies >= currentMaxCandy) {
                hasGreatestNumberOfCandiesArray[i] = true;
            } else {
                hasGreatestNumberOfCandiesArray[i] = false;
            }
        }

        return Arrays.asList(hasGreatestNumberOfCandiesArray);
    }

    public int findCurrentMaxCandy(int[] candies) {

        int currentMaxCandy = 0;

        for(int i: candies) {

            if (i > currentMaxCandy) {
                currentMaxCandy = i;
            }
        }
